...
# serializers package data and unpackage it in some sort of type that we can use (json for example)
# 2 types of serializers: 
# general serializer (class) and module serializer (less customizable and much simpler to use)

class profileS(serializers.ModelSerializer):
    class Meta:
        model = profile
        fields = ['picture', 'birthDate'] # fields that are going to be serialized (packaged)
